        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        {relationships && (
                            <p className="text-slate-400 text-sm mt-1">{relationships.asset_name}</p>
                        )}
                    </div>
                </div>
                <button
            ...
            {showAddForm && (
                <div className="glass-panel p-6">
                    <h3 className="text-lg font-bold text-white mb-4">Add New Relationship</h3>
                    <form onSubmit={handleAddRelationship} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm text-slate-400 mb-2">Target Asset</label>
                            <select
                                required
                                value={newRelationship.target_asset_id}
                                onChange={(e) => setNewRelationship({...newRelationship, target_asset_id: e.target.value})}
                                className="w-full bg-slate-800/50 border border-white/10 rounded-lg p-2.5 text-slate-200"
                            >
                        ...
                        <div>
                            <label className="block text-sm text-slate-400 mb-2">Relationship Type</label>
                            <select
                        ...
                        <div>
                            <label className="block text-sm text-slate-400 mb-2">Criticality (1-5)</label>
                            <input
                                type="range"
                                min="1"
                                max="5"
                        ...
                        <div className="md:col-span-2 flex justify-end space-x-3">
                            <button type="button" onClick={() => setShowAddForm(false)} className="btn bg-slate-700 hover:bg-slate-600 text-white">
                                Cancel
                            </button>
            ...
            <div className="glass-panel min-h-[400px] p-8">
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-64 text-center">
                        ...
                        <h3 className="text-xl font-bold text-white mb-2">Error Loading Relationships</h3>
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="bg-slate-900/50 p-6 rounded-xl border border-white/10">
                                <h4 className="font-bold text-slate-200 mb-4 border-b border-white/5 pb-2 flex items-center">
                                    <span className="w-3 h-3 bg-orange-500 rounded-full mr-2"></span>
                                    Upstream (This Asset Depends On)
                                </h4>
                                <ul className="space-y-3">
                                    {relationships.upstream.map(rel => (
                                        <li key={rel.id} className="flex items-center justify-between p-3 bg-white/5 rounded-lg group">
                                            <div className="flex items-center space-x-3">
                                                ...
                                                <Link href={`/assets/${rel.related_asset?.id}`} className="text-slate-300 hover:text-white">
                                                    {rel.related_asset?.name}
                                                    <span className="text-xs text-slate-500 ml-2">({rel.related_asset?.type})</span>
                                                </Link>
                                            </div>
                                            <button
                                    ...
                                    {relationships.upstream.length === 0 && (
                                        <li className="text-slate-500 italic text-center py-4">No upstream dependencies</li>
                                    )}
                                </ul>
                            </div>
                            ...
                            <div className="bg-slate-900/50 p-6 rounded-xl border border-white/10">
                                ...
                                <ul className="space-y-3">
                                    ...
                                    {relationships.downstream.length === 0 && (
                                        <li className="text-slate-500 italic text-center py-4">No downstream dependencies</li>
                                    )}
                                </ul>
                            </div>
                        </div>
